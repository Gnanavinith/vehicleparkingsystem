...
const css = `
  ...
  *, *::before, *::after { box-sizing:border-box; }
  ...
  .tb-select {
    ...
    appearance:none; -webkit-appearance:none; transition:all .15s;
    letter-spacing:.02em;
  }
  ...
  .tb-dropdown {
    ...
    animation:tb-drop .16s cubic-bezier(.22,1,.36,1);
    overflow:hidden;
  ...
  .tb-dd-title {
    ...
    font-size:15px; color:#0a0a0a; letter-spacing:'-0.01em';
  }
  .tb-clear-btn {
  ...
  .tb-chevron { color:#aaa; transition:transform .18s, color .14s; flex-shrink:0; }
  .tb-profile-btn.tb-open .tb-chevron { transform:rotate(180deg); color:rgba(255,255,255,.5); }
  ...
  .tb-profile-info-name {
    font-family:'DM Serif Display',serif; font-style:italic;
    font-size:15px; color:#0a0a0a; letter-spacing:'-0.01em'; line-height:1.2;
  }
        ...
        <div className="tb-right">
          ...
          <div style={{ position:'relative' }} ref={profileRef}>
            ...
            >
              ...
              <span className="tb-profile-name">{user?.name || 'Staff Member'}</span>
              <FaChevronDown size={10} className="tb-chevron" style={{ color: showProfile ? 'rgba(255,255,255,.4)' : '#aaa', transition:'transform .18s', transform: showProfile ? 'rotate(180deg)' : 'none' }} />
            </button>

            {showProfile && (
              <div className="tb-dropdown tb-profile-dd">
                <div className="tb-profile-info">
                  <div className="tb-profile-info-name">{user?.name || 'Staff Member'}</div>
                  <div className="tb-profile-info-email">{user?.email || 'staff@parkingpro'}</div>
                  <span className="tb-role-chip">
                    <FaShieldAlt size={7} /> {roleLabel}
                  </span>
                </div>
